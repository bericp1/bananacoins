...
export function usePlayersAndScores() {
  ...
  const [teamsWithPlayers, setTeamsWithPlayers] = useState<Set<number>>(new Set());
  ...
  useEffect(() => {
    const teams = new Set(
      players
      .filter((player): player is Player & { team: number } => player.team !== null)
  ...
  const fetchScores = async () => {
    ...
    const processedPlayerScores = Object.values(playerScoresMap).map((playerScore) => {
  ...
  const deletePlayer = async () => {
    ...
    if (error) {
      ...
      setPlayers(prev => prev.filter(player => player.uuid !== playerToDelete.uuid))
    }
  ...
  const updatePlayerScore = async (uuid: string, score: number) => {
    ...
    if (error) {
      ...
      setPlayers(prev => prev.map(p => p.uuid === uuid ? { ...p, score } : p))
    }
  }
  ...
  const resetPlayerTeam = async (uuid: string) => {
    ...
    if (error) {
      ...
      setPlayers(prev => prev.map(p => p.uuid === uuid ? { ...p, team: null, score: 0 } : p))
    }
  }
  ...
  const resetAllScoresAndTeams = async () => {
    const { error } = await supabase
      ...
      .in('uuid', players.map((player) => player.uuid))
  ...
  const confirmAndSaveScores = async () => {
    ...
    const nextRound = maxRoundData && maxRoundData.length > 0 ? maxRoundData[0].round + 1 : 1

    const teamAverages = players.reduce((acc, player) => {
      if (player.team !== null) {
        if (!acc[player.team]) {
    ...
    const teamScores = Object.entries(teamAverages).reduce((acc, [team, { total, count }]) => {
  ...
  const areAllTeamScoresValid = () => {
    const teamsWithPlayers = new Set(players.filter(player => player.team !== null).map(player => player.team));
    return teamsWithPlayers.size >= 2 && players.every(player => player.team === null || (player.score > 0 && player.score !== null));
  };
